...
public class UserController {
	...
	public ResponseEntity<Object> login(@RequestParam("email") String email,@RequestParam("password") String password) {
		...
		User fetchUser = null;
		try{
			fetchUser = userService.findByEmail(email);
		} catch (Exception e) {
			//throw new NoSuchElementException("User not registered");
		}
		
		fetchUser = userService.loginUser(email,password);
	...
	public ResponseEntity<Object> resetPassword(@RequestParam("oldPassword") String oldPassword,@RequestParam("newPassword") @Size(min=8,message = "Password should contain greater than 8 character") String newPassword,@PathVariable("email") String email,@ApiIgnore HttpSession session){
		
		User userFetch = null;
		
		try{
			userFetch = userService.findByEmail(email);
		} catch (Exception e) {
			throw new NoSuchElementException("User not registered");
		}
		
		userFetch = userService.loginUser(email,oldPassword);
